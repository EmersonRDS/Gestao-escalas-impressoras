...
<main class="content">
	<div class="container-fluid p-0">
        <?php
            ...
            if(isset($_POST['cadImpressora'])){
                ...
                if(isset($_POST['nDias']) and isset($_POST['nColaboradores'])){
                    ...
                    try {          
                        ...
                        $comandoSQL = "INSERT INTO tb_escala (id_usuario,setor_escala,loja_escala ) VALUES ('$idUsuario','$setor','$loja')"; 
                        $grava = $conexao->prepare($comandoSQL); //testa o comando
                        $grava->execute(array());
            ...
            }else if(isset($_POST['btn_altera_escala'])){
                
                //-------------------------------------------------------------
                //Alterar escala
                //-------------------------------------------------------------

                $nDias = count($_POST['data']);
                $id = $_POST['id'];
                

                if(isset($_POST['novoSetor']) || isset($_POST['novaLoja'])){
                    $novoSetor;
                    $novaLoja;
                    $update;
                    if(isset($_POST['novoSetor'])){
                        $novoSetor = $_POST['novoSetor'];
                        $update = $conexao->prepare("UPDATE tb_escala SET setor_escala = '$novoSetor' WHERE id = '$id';");
                        $update->execute();
                    }
                    if(isset($_POST['novaLoja'])){
                        $novaLoja = $_POST['novaLoja'];
                        $update = $conexao->prepare("UPDATE tb_escala SET loja_escala = '$novaLoja' WHERE id = '$id';");
                        $update->execute();
                    }
                    ...
                    $deletar = $conexao->query("DELETE FROM tb_semana WHERE id_escala = '$id';");
                        for($i=0; $i<$nDias ; $i++){
                            if($dia = $_POST["data"][$i] != ''){
                                $dia = $_POST["data"][$i];
                                ...
                                f_pessoa_8,pessoa_9,f_pessoa_9,pessoa_10,f_pessoa_10,pessoa_11,f_pessoa_11) 
                                VALUES 
                                ('$id' , '$dia' , '$colaboradores[0]' , '$folgaColaboradores[0]' ,
                                '$colaboradores[1]' , '$folgaColaboradores[1]' , '$colaboradores[2]' , '$folgaColaboradores[2]' ,
                                '$colaboradores[3]' , '$folgaColaboradores[3]' , '$colaboradores[4]' , '$folgaColaboradores[4]' , 
                                '$colaboradores[5]' , '$folgaColaboradores[5]' , '$colaboradores[6]' , '$folgaColaboradores[6]' ,
                                '$colaboradores[7]' , '$folgaColaboradores[7]' , '$colaboradores[8]' , '$folgaColaboradores[8]' ,
